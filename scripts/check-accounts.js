...
(async () => {
  try {
    ...
    if (!users.length) { logger.info('（无用户数据）'); return; }
    ...
    for (const u of users) {
      ...
      logger.info('账本列表：', books.map(b => `#${b.id} ${b.name}${b.is_default ? '(默认)' : ''}`).join('  |  ') || '（无账本）');
      ...
      logger.info('账户分布（按账本 / 状态）：');
      for (const r of dist) {
        logger.info(`  book_id=${r.book_id ?? 'NULL'}  status=${r.status}  数量=${r.cnt}`);
      }
      ...
      for (const a of orphan) logger.info(`   id=${a.id}  ${a.name}  book_id=${a.book_id ?? 'NULL'}`);
    ...
    logger.error('请确认在能连上数据库的环境运行（docker 部署用 `docker compose exec app node scripts/check-accounts.js`）。');
  } finally {
    process.exit(0);
  }
})();
