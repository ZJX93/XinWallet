...
(async () => {
  try {
    ...
    if (!users.length) { console.log('（无用户数据）'); return; }
    ...
    for (const u of users) {
      ...
      console.log('账本列表：', books.map(b => `#${b.id} ${b.name}${b.is_default ? '(默认)' : ''}`).join('  |  ') || '（无账本）');
      ...
      console.log('账户分布（按账本 / 状态）：');
      for (const r of dist) {
        console.log(`  book_id=${r.book_id ?? 'NULL'}  status=${r.status}  数量=${r.cnt}`);
      }
      ...
      for (const a of orphan) console.log(`   id=${a.id}  ${a.name}  book_id=${a.book_id ?? 'NULL'}`);
