...
function reportKpi() {
  console.info('=== KPI 卡：各屏宽下卡内可用 ===');
  for (const s of SCREENS) {
    console.info(`  ${s}vp 屏 → ${CONTAINERS.kpiCard(s)}vp`);
  }
  console.info('');
  console.info('=== 20sp 下各金额能否放下 ===');
  console.info('  值'.padEnd(20) + '宽度' + SCREENS.map(s => String(s).padStart(6)).join(''));
  for (const v of MONEY_CASES) {
    const ww = width(v, 20);
    const cells = SCREENS.map(s => (ww <= CONTAINERS.kpiCard(s) ? '    ok' : '    XX')).join('');
    console.info('  ' + v.padEnd(18) + String(ww).padStart(6) + cells);
  }
  console.info('');
  console.info('=== minFontSize 倒推（最窄的 320vp 屏）===');
  const avail = CONTAINERS.kpiCard(320);
  for (const v of ['¥1,234,567.89', '-¥1,234,567.89', '¥12,345,678.90']) {
    const fs = fitFontSize(v, avail);
    console.info(`  ${v.padEnd(18)}→ ${fs}sp（可用 ${avail}，实宽 ${width(v, fs)}）`);
  }
  console.info('');
  console.info('  当前取 minFontSize(15)：留一档余量，不按亿级定');
  console.info('  （按理论最大值定会让常见值也被压小，为极端情况牺牲日常可读性）');
  console.info('');
  console.info('=== 环形图中心（可用 ' + CONTAINERS.donutCenter() + 'vp，与屏宽无关）===');
  for (const v of MONEY_CASES) {
    const ww = width(v, 20);
    console.info('  ' + String(ww).padStart(6) + '  ' + v.padEnd(18) +
