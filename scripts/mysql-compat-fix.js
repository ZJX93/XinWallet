...
function processFile(filePath) {
  ...
  if (newContent !== before) {
    ...
    if (count > 0) {
      console.info(`  [占位符] ${filePath}: 替换 ${count} 处 $N → ?`);
      modified = true;
      changes += count;
    }
  }
  ...
  if (newContent !== before2) {
    ...
    if (count > 0) {
      console.info(`  [INTERVAL] ${filePath}: 替换 ${count} 处 INTERVAL 'N unit' → INTERVAL N UNIT`);
      modified = true;
      changes += count;
    }
  }
...
function main() {
  console.info('=== MySQL 兼容性修复 ===');
  if (DRY_RUN) console.info('[DRY RUN 模式 — 不写入文件]\n');

  const files = getJsFiles(SERVER_DIR);
  console.info(`扫描 ${files.length} 个 JS 文件...\n`);
  ...
  console.info(`\n完成！共替换 ${total} 处。`);
  if (DRY_RUN) console.info('(以上为预览，运行时不带 --dry-run 才会写入)');
}
