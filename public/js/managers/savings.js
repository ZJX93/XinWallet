...
const SavingsGoalManager = {
    ...
    populateAccounts() {
        ...
        sel.innerHTML = `<option value="">${escapeHtml(tt('goal.select.savingsAccount', '请选择储蓄账户 *'))}</option>`
            + accountOptionsHtml(cache.accounts, { withBalance: true });
    },
    populateSourceAccounts() {
        const sel = document.getElementById('goalSource');
        sel.innerHTML = `<option value="">${escapeHtml(tt('goal.select.sourceAccount', '请选择来源账户 *'))}</option>`
            + accountOptionsHtml(cache.accounts, { withBalance: true });
    },
    edit(id) {
    ...
    openAmountModal(id, type) {
        ...
        accSel.innerHTML = `<option value="">${escapeHtml(tt('goal.amount.selectAccount', '-- 请选择账户 * --'))}</option>` +
            accountOptionsHtml(cache.accounts, { withBalance: true, excludeId: g.account_id });
