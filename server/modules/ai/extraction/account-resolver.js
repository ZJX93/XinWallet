...
const KEYWORD_GROUPS = [
    {
        ...
        //
        // ⛔⛔【顺序即优先级，子渠道必须排在总渠道前面】
        //     scanPaymentChannels 取【第一个命中】的 kw 作为 keyword，而
        //     findAccountByChannel 的优先级 0 正是拿这个 keyword 去账户名里匹配。
        //       · 子渠道在前 ['花呗', …, '支付宝']：
        //         票据「付款方式 花呗」+「商品说明 支付宝缴款」→ keyword='花呗'
        //         → 匹配账户名含「花呗」的「支付宝花呗」 ✅
        //       · 总渠道在前（旧写法）['支付宝', …, '花呗']：
        //         同一张票据先命中泛词 '支付宝' → 匹配到【第一个】含「支付宝」的账户
        //         → 实测（2026-09-07 用户截图）落到「支付宝余额」❌，
        //           花呗账单被记成余额付款（账户错 → 余额静默错账）。
        kws: ['花呗', '借呗', '信用购', '余额宝', '集分宝', '支付宝', 'alipay'],
    },
    {
        source: 'wechat',
        label: '微信',
        /*  ⛔ 必须带「零钱 / 零钱通」：微信账单详情页的支付方式字段只写这两个词，
            ...
            ⛔ 同理【零钱通要排在「零钱」和「微信」之前】：票据写「零钱通」时若先
               命中泛词「微信」，就会匹配到「微信零钱」而不是「微信零钱通」。 */
        kws: ['零钱通', '零钱', '微信', 'wechat', 'weixin'],
    },
    {
        source: 'union',
        label: '云闪付',
