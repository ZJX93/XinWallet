...
async function healSchemaColumns() {
  ...
  // 多币种 P2-2d：持仓货币（独立于关联账户），默认 CNY；老库补齐
  await ensureColumn('investments', 'currency', "VARCHAR(3) NOT NULL DEFAULT 'CNY'");
  // 多币种 P2-2d：理财流水货币（跟随持仓币种，默认 CNY）；老库补齐
  await ensureColumn('investment_transactions', 'currency', "VARCHAR(3) NOT NULL DEFAULT 'CNY'");
  // 多币种 P2-2d：债务还款流水货币（跟随债务币种，默认 CNY）；老库补齐
  ...
  // 多币种折算痕迹：外币消费折成账户币种入账时记录原币与汇率（老库补齐，新库由 schema CREATE TABLE 覆盖）
  await ensureColumn('transactions', 'original_amount', 'DECIMAL(15,2) DEFAULT NULL');
  await ensureColumn('transactions', 'original_currency', 'VARCHAR(3) DEFAULT NULL');
  await ensureColumn('transactions', 'exchange_rate', 'DECIMAL(18,6) DEFAULT NULL');
  await ensureColumn('transactions', 'rate_date', 'DATE DEFAULT NULL');
