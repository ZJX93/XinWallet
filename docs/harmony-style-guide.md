...
## 65. ⛔ `Path()` 只有 `fill` 通道可靠，`stroke` 通道不要用

底栏图标连改 6 轮才定案，根因是 ArkUI 的 `Path` 与 SVG 语义不等价。

### 三代踩坑

1. **`Image($r(...)).fillColor()`** —— 只改 `fill` 属性。对 `fill="none" stroke="#000"` 的描边式 SVG，
   会把 `fill="none"` 覆盖成实色 → 小票变整块深色、饼图变实心圆。**不报错、静默错**。
2. **`Path().fill(Color.Transparent).stroke(c).strokeWidth(1.8)`** —— 真机渲染成**小黑方块/小黑圆点**。
   `fill()` 签名是 `fill(value: ResourceColor)`，传 `Color.Transparent` 能过编译，
   但**不等价于 SVG 的 `fill="none"`**，闭合子路径仍被默认色填满。
   且 `Path` **没有 `viewPort()`**（只有 `Shape` 有），`commands` 是 vp 绝对坐标、不缩放。
3. **改成实心色块** —— 与参照图的描边线性风格不符，被驳回（「不要自己创新」）。

### ✅ 正解：全程 `fill(color)` + `strokeWidth(0)`，用几何构造出描边观感

项目内唯一真机验证通过的范式是 `Search.ets:FunnelIcon`。两种构造法按图形选：

- **闭合环（圆、圆角框）** → 外轮廓顺时针 + 内轮廓逆时针，按 nonzero winding 挖空。
  圆必须用两段半圆弧 `A` 命令拼（`commands` 不支持 `<circle>`）；
  内圆 sweep 取 0（与外圆的 1 相反）才是逆时针。
- **带折线/锯齿/波浪的轮廓** → **别用挖空法**（转角处法向宽度极易算错、缺口难调）。
  改成**把每段笔画画成独立实心细带，各带一律顺时针** —— nonzero 下同号叠加，
  重叠处只会更实、不会互相抵消，转角天然无缺口。相邻带刻意重叠约 1vp 补角。
  ⚠️ 新增笔画带必须顺时针，逆时针会与相邻带抵消挖出白洞。
  斜边细带做法：外缘折线 L→R、内缘折线 R→L 整体沿 y 平移 d，则法向宽 ≈ d·cos θ（斜率越大越细，需补偿 d）。

环宽 **1.7vp** 与同栏 `SymbolGlyph` 24vp 的笔画重量对齐。

**验证技巧：把同一份 `d` 数据贴进 SVG 放大 6 倍先看形状**，比装机快得多。

### 底栏图标形状已冻结（用户第 6 轮拍板，勿再改）

中间两轮按口述改的「顶锯齿 + 底卷筒」「贯穿竖线 + 实心扇区」全部废弃 ——
用户直接贴回第 3 轮的渲染预览 + 「还是这版吧」定案。

**账单 = 小票**：顶边平直 + 底边 3 个撕口锯齿 + 中间 2 条条目横线。4 条子路径：

```
外轮廓(顺)  M5 3 L19 3 L19 21 L16.5 19.2 L14 21 L11.5 19.2 L9 21 L6.5 19.2 L5 20.3 Z
内轮廓(逆)  M6.7 4.7 L6.7 17.55 L9 19.2 L11.5 17.4 L14 19.2 L16.5 17.4 L17.3 17.98 L17.3 4.7 Z
+ 2 条横线细矩形(顺)
```

**统计 = 饼图**：圆环（外 r=9.5 顺 / 内 r=7.8 逆挖空）+ **12 点竖半径 + 3 点横半径**两条细矩形，
切出右上一块。**不是**实心扇区，也**不是**贯穿全圆的竖线。

**选中态**：描边线性图标 + 10% 品牌色纯平圆底（34vp 圆），**无阴影、无放大、无加粗**。

### 流程教训

形状类需求靠文字来回描述成本极高（连改 3 轮没对上）。
**正解：一次给 2–3 个候选形状的渲染图让用户挑**，别改一版渲染一版等反馈。

### 跨端现状

鸿蒙 `media/*.svg` 14 个是唯一在用 svg 的端；安卓 `res/drawable/` 是空的（全走 `material-icons-extended`）；
Web 用 emoji。**SVG 文件不跨端，能跨端的只有 path 的 `d` 数据。**
